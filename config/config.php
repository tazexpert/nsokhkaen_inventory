...
// APP_URL / APP_NAME are editable at runtime from the "ตั้งค่าระบบ" (Settings)
// menu (admin only) and stored in the `settings` table. These are just the
// fallback values used the first time, or if that table hasn't been created
// yet (run database/migrations/002_add_settings_and_storage_locations.sql).
$appUrl = getenv('APP_URL') ?: 'http://localhost/nsokhkaen_inventory/';
$appName = 'ระบบบริหารจัดการวัสดุและครุภัณฑ์ สำนักงานสถิติจังหวัดขอนแก่น';

try {
    $settingsStmt = getDbConnection()->query('SELECT `key`, `value` FROM settings');
    foreach ($settingsStmt->fetchAll() as $row) {
        if ($row['key'] === 'app_url' && $row['value'] !== '') {
            $appUrl = $row['value'];
        } elseif ($row['key'] === 'app_name' && $row['value'] !== '') {
            $appName = $row['value'];
        }
    }
} catch (Throwable $e) {
    // `settings` table not migrated yet - silently keep the defaults above
}
unset($settingsStmt, $row);
